 ...
 *
 * Kuvaus:
 * Book moduuli toteuttaa kirjojen tallettamisen stl vector tyyliseen tietovaras
 * toon jossa on Chapter structeja linkitettynä toisiinsa osoittimia käyttäen
 * Moduulin public osassa on funktioita joita muut projektin moduulit
 * hyödyntävät sekä private osa joita tämä moduuli hyödyntää
 *
...
private:
    ...
    //Data vektori johon kaikki kirjan luvut alussa talletetaan
    //vektorin alkiot ovat pareja joissa "avain" on luvun id
    //ja parina itse luku chapter struct muodossa
    using Data = std::vector<std::pair<std::string,Chapter*>>;
    //vektori johon talletetaan tietoa pointtereista metodien aikan
    using chapVec = std::vector<Chapter*>;
    //map rakenne
    using IdMap = std::map<std::string, std::string>;
    //vaaditut Error viestit
    std::string NOTFOUND = "Error: Not found: ";
    std::string ERRORN = "Error. Level can't be less than 1.";

    //vektori johon luvut talletetaan
    Data chapters_;
    ...
    //tarkastaa onko ID kirjassa
    bool chapterExists(const std::string &id) const;
    //rekursiivinen lukujen printtaus, printtaa kaikki luvut
    void printChaptersRecursive(Chapter *ch,
                                int index,
                                const std::string &indentation) const;
    //muuttaa vektorin alkiot map rakenteeseen jotta saadaan tulostettua
    //aakkosjärjestyksessä
    IdMap unorderedToOrdered() const;
    //tulostaa setin alkiot aakkosjärjestyksessä
    void printSet(const std::set<std::string> &idSet) const;
    //laskee rekursiivisesti halutun luvun pituuden
    int recursiveCount(Chapter *ptr) const;
    //etsii osoittimen missä on suurin luku pituudeltaan
    Chapter *recursiveLongestChapter(Chapter*ptr) const;
    //etsii osoittimen missä on pienin luku pituudeltaan
    Chapter *recursiveShortestChapter(Chapter*ptr) const;
    //etsii halutun luvun vanhemmat rekursiivisesti
    std::vector<Chapter*> parentsToVectorRecurisve(Chapter *ptr,
                                                   int level) const;
    //etsii halutun luvun aliluvut rekursiivisesti
    std::vector<Chapter*> subsToVectorRecurisve(Chapter *ptr,
                                                int level) const;
    //apuna parentsN ja subChaptersN tulostamiseen
    void printParentOrSub(Chapter* ptr, chapVec &chapVec,
                          std::string &parentOrSub) const;
    //etsii halutut aliluvut ja muodostaa niistä vektorin
    std::vector<Chapter*> subsToVector(Chapter *ptr) const;
};
